...
public final class KakaoRestaurantSearchSource implements RestaurantSearchSource {
    ...
    public KakaoRestaurantSearchSource(@Value("${anymeal.kakao-restapi-key}") String kakaoRestapiKey) {
    ...
    public Optional<Restaurant> searchNear(String x, String y) {
        ...
        log.debug("KakaoRestaurantSearchSource searchNear restaurants: {}", restaurants.size());
        if (restaurants.size() > 1) {
            final var index = Math.abs(secureRandom.nextInt() % restaurants.size());
            return Optional.of(Restaurant.withSource(restaurants.get(index), "kakao"));
        }
        return restaurants.stream()
                .findAny()
                .map(r -> Restaurant.withSource(r, "kakao"));
    }

    private List<Restaurant> getRestaurantNearAllPages(Point point, String sort, int radius,
            String categoryCode, int page) {
        final var categoryResponse = getRestaurantNear(point, sort, radius, categoryCode, page);
    ...
    private CategoryResponse getRestaurantNear(Point point, String sort, int radius,
            String categoryCode, int page) {
